...
public class Robot extends TimedRobot implements RobotProperties {
  ...
  public void robotPeriodic() {
    ...
    SmartDashboard.putString("Lower Shooter RPM:", String.format("%.2f | %.2f", shooterController.getLowerShooterVelocity(), shooterController.getLowerShooterTargetVelocity()));
    SmartDashboard.putString("Upper Shooter RPM:", String.format("%.2f | %.2f", shooterController.getUpperShooterVelocity(), shooterController.getUpperShooterTargetVelocity()));
  ...
  private void operatorControlsPeriodic(final XboxControllerState operatorControllerState) {
    ...
    // Shooter Control
    final boolean shooterTiltWithinRange = Math.abs(Get_Gyro_Displacement(shooterController.getShooterTilt(), shooterController.getShooterTiltSetPosition())) < SHOOTER_TILT_ALLOWED_DEVIATION;
